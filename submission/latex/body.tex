...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../../figures/route_gap_uncertainty_framework.png}
\caption{Route-gap uncertainty workflow for animal tracking data. A prolonged GPS gap removes part of an observed migratory route, creating uncertainty that is hidden if the track is reduced to a single completed curve. The workflow reconstructs plausible missing-route alternatives, validates and calibrates their uncertainty envelopes against withheld observations, and propagates the resulting uncertainty into distance matrices, cluster stability, anomaly rankings, and uncertainty summaries.}
