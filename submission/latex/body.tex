...
\caption{Route-gap uncertainty workflow and diagnostic outputs for incomplete animal tracking data. Panel (a) shows an observed migration route with a contiguous withheld GPS gap. Panel (b) compares deterministic spherical-bridge reconstruction with stochastic Brownian bridge samples for the same missing interval. Panel (c) shows withheld-segment coverage before calibration and the calibrated coverage target. Panel (d) shows downstream stability at 60\% gaps, with distance-matrix rank correlation shown as bars and cluster agreement shown as points.}
